...
* **Launch / Self-Serve Plans**: Use [branching-and-merging.md](team-collaboration/branching-and-merging.md "mention") and create separate branches for dev, stage, and prod. Your frontend should typically only be calling production endpoints, but you can also deploy a second frontend for testing that defaults to stage or dev
* **Scale or Enterprise Plans with Xano Link**: Deploy your changes to separate workspaces using [xano-link.md](xano-features/advanced-back-end-features/xano-link.md "mention")
* **Enterprise Plans with Tenant Center**: Create separate tenants in your [tenant-center.md](enterprise/enterprise-features/tenant-center.md "mention") for each environment, and deploy releases to them
{% endstep %}

{% step %}
### Build Tests

You should always be building tests for your function stacks using our [unit-tests.md](testing-debugging/unit-tests.md "mention") and [test-suites.md](testing-debugging/test-suites.md "mention") features. Unit Tests are designed to run a test on a single function stack (such as a sign-up API), and Test Suites (Workflow Tests) are designed to check what would be a typical multi-step flow for your application (such as a user signing up, purchasing a subscription plan, and receiving a confirmation email).
...
Deploy your changes to Stage and run your [unit-tests.md](testing-debugging/unit-tests.md "mention") and [test-suites.md](testing-debugging/test-suites.md "mention") to ensure everything is behaving as expected.
...
[test-suites.md](testing-debugging/test-suites.md "mention") create a duplicate copy of your database, temporarily, just for testing. So, if your production database is large or complex, you may have trouble completing your tests or experience additional complications. It is strongly recommended to utilize [data-sources.md](the-database/database-basics/data-sources.md "mention") to navigate around this potential issue.
...
If you're working with external services, you may require different configurations between your development, stage, and production environments, such as different API keys. Typically, you'd store these in your [environment-variables.md](the-function-stack/environment-variables.md "mention").

* **If you're using Branching/Merging**, you'll be limited to storing all environment variables together. You can use statements like [#get-environment-variables](the-function-stack/functions/utility-functions.md#get-environment-variables "mention") to manually parse all available environment variables and retrieve the ones necessary depending on the branch you're on.
...
If you find that multiple team members need to work on the same function stacks, we have [Broken link](broken-reference "mention") features built right into Xano to ensure that the experience is as smooth as possible.

If your plan has access to RBAC (Role-based Access Control), it is imperative that you manage permissions properly to ensure that only specific team members can push changes from development to stage, and even more so from stage to production. Be sure to review our documentation on [role-based-access-control-rbac.md](team-collaboration/role-based-access-control-rbac.md "mention") for more information.

### Using Mock Responses for Test-Driven Development

For certain steps, you should also be [#mocking-responses](testing-debugging/unit-tests.md#mocking-responses "mention") both for the sake of speed and consistency when running your tests.
